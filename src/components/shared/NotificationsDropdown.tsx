      ...
      {open && (
        <div className="absolute right-0 top-10 w-80 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl shadow-black/20 dark:shadow-black/40 z-50 overflow-hidden"
          ...
          <div className="notif-scroll max-h-72 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800/60">
            {items.length === 0 && (
              <p className="text-center text-xs text-gray-500 py-8">No notifications</p>
            )}
            {items.map((n) => (
              <button key={n.id} onClick={() => { handleMarkRead(n.id); setOpen(false); }}
                ...
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 bg-violet-900/50 text-violet-400">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-xs font-semibold truncate ${n.read ? "text-gray-500 dark:text-gray-400" : "text-gray-900 dark:text-white"}`}>{n.title}</p>
          ...
          <div className="px-4 py-2.5 border-t border-gray-100 dark:border-gray-800">
            <a href="/admin/notifications" onClick={() => setOpen(false)}
