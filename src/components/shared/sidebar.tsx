...
const navItems = [
  ...
  {
    label: "Roles Management", href: "/admin/roles", badge: true,
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {AllowedRoutes.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={` ${item.label === "Roles Management" && user?.role != "super_admin" ? "hidden" : "flex"} items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all group relative
