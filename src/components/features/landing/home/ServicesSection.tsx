...
import { getServices } from "@/src/api/services/service";
...
function getGradient(key: string) {
  return GRADIENT_MAP[key] ?? "from-violet-600 to-purple-700";
}
    ...
    <section id="services" className="py-24 bg-white dark:bg-transparent">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((s, i) => {
            const gradient = getGradient(s.color_gradient);
            return (
                ...
                <div className="flip-card h-72 w-full cursor-pointer">
                  <div className="flip-card-inner">
                    <div className="flip-card-front relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 flex flex-col overflow-hidden">
                      ...
                      <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center text-white mb-5 shadow-lg float-anim`} style={{ animationDelay: `${i * 0.4}s` }}>
                      ...
                      <p className={`mt-auto text-xs font-medium bg-gradient-to-r ${gradient} bg-clip-text text-transparent`}>Hover to see more →</p>
                    </div>
                    <div className={`flip-card-back relative bg-gradient-to-br ${gradient} rounded-2xl p-6 flex flex-col justify-between overflow-hidden`}>
